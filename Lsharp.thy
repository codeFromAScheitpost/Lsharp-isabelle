...
"hypothesis (S,F,T) t =
    (\<forall> s \<in> S. \<forall> i. \<exists> tran op n out. (run T s = Some (Node tran,op)) \<and> (tran i = Some (n,out)) \<and>
      (if (s @ [i]) \<in> S
        then t (s,i) = (s @ [i],out)
...
lemma otree_eq_out: "case run (Node r) i of
...
lemma otree_eq_out_exhaust: "case run T i of
...
lemma out_eq_otree: "\<exists> node. (case outs_run (Node r) i of
...
lemma otree_split: "run (Node r) (a # acc) = Some (tr1,out1) \<Longrightarrow> r a \<noteq> None"
  by (auto split: prod.splits option.splits)


lemma out_split: "outs_run (Node r) (a # acc) = Some (out1) \<Longrightarrow> r a \<noteq> None"
  by (auto split: prod.splits option.splits)
...
proof -
  ...
  then have nothing_none_otree: "\<forall> s \<in> S. \<forall> i. run T (s @ [i]) \<noteq> None"
    by (smt (verit) case_optionE option.discI otree_eq_out run.elims)
...
lemma output_query_retains_some_specific:
  assumes "length ip = length op" and
    "run (Node r) acc = Some (tr1,out1)" and
    "run (process_output_query (Node r) ip op) acc = Some (tr2,out2)"
  shows "out1 = out2"
...
next
  ...
  next
    ...
    then show ?thesis proof (cases "i = a")
      case True
      then have "run (process_output_query (Node r) ip op) (a # acc) =
          run (process_output_query (Node r) (i # ilist) (ops # olist)) (a # acc)"
        using Cons op
        by presburger
      also have "\<dots> = run (case r i of
          None \<Rightarrow> (Node (\<lambda> j. if j = i
        ...
        using True ra
        by simp
      also have "\<dots> = run (Node (\<lambda> j. if j = i
          then Some ((process_output_query (Node r2) ilist olist,outs))
          else r j)) (a # acc)"
        using ra
        by (simp add: True)
      also have "\<dots> = (case run (process_output_query (Node r2) ilist olist) acc of
          Some (node,output) \<Rightarrow> Some (node,outs # output) |
          None \<Rightarrow> None)"
        using ra True
        by auto
      finally have calc1: "run (process_output_query (Node r) ip op) (a # acc) =
          (case run (process_output_query (Node r2) ilist olist) acc of
            Some (node,output) \<Rightarrow> Some (node,outs # output) |
            None \<Rightarrow> None)"
        by blast
      have "run (process_output_query (Node r2) ilist olist) acc \<noteq> None"
        using calc1 c Cons True
        by (metis not_Some_eq option.simps(4))
      then obtain node1 outputs1 where
        n1: "run (process_output_query (Node r2) ilist olist) acc = Some (node1,outputs1)"
        by fast
      have calc2: "run (Node r) (a # acc) = (case run (Node r2) acc of
          Some (node,output) \<Rightarrow> Some (node,outs # output) |
          None \<Rightarrow> None)"
        using c ra True
        by simp
      then have "run (Node r2) acc \<noteq> None"
        using c
        by (metis not_Some_eq option.simps(4))
      then obtain node2 outputs2 where
        n2: "run (Node r2) acc = Some (node2,outputs2)"
        by auto
    ...
    next
      ...
      also have "\<dots> = run (case r i of
        ...
        using False ra
        by simp
      also have "\<dots> = (case run (Node r2) acc of
          Some (node,output) \<Rightarrow> Some (node,outs # output) |
          None \<Rightarrow> None)"
        using ra
        apply (cases "r i")
        using False
        by auto
      also have "\<dots> = run (Node r) (a # acc)"
        using ra
        by simp
      finally show ?thesis
        using c Cons
...
lemma output_query_retains_some_specific_output:
  ...
  shows "Some out1 = outs_run (process_output_query (Node r) ip op) acc"
...
lemma output_query_keeps_invar:
  ...
  shows "outs_run (process_output_query T j out) i \<noteq> None \<longrightarrow> outs_run (process_output_query T j out) i = Some (output_query M i)"
proof -
  obtain t q_0 where
    "M = (q_0,t)"
    by fastforce
...
lemma max_size_S_aux:
  assumes "func_sim M T f" and
    "sapart (S,F,T)" and
    "finite S"
  shows "card S \<le> card (UNIV :: 's set)"
proof (rule ccontr)
  assume ass: "\<not> card S \<le> card (UNIV :: 's set)"
  then have gt: "card S > card (UNIV :: 's set)"
    by simp
  obtain c where
    c: "card (UNIV :: 's set) = c"
    by simp
  have "\<forall> s \<in> S. f s \<in> (UNIV :: 's set)"
    by simp
...
next
  ...
  then have d: "\<forall> j. outs_run (process_output_query T (s @ [i]) out) j \<noteq> None \<longrightarrow>
    ...
    by (smt (verit) invars lens outs_run.elims output_query_keeps_invar output_query_retains_some_specific_output)
...
next
  ...
  then have d: "\<forall> j. outs_run (process_output_query T (f @ w) out) j \<noteq> None \<longrightarrow>
    ...
    by (smt (verit,del_insts) invars lens outs_run.elims output_query_retains_some_specific_output)
...
theorem algo_step_increases_norm:
  assumes "algo_step ((S :: 'in list set),F,T) (S',F',T')" and
    "invar (S,F,T)"
...
lemma hypothesis_no_step_output_same:
  assumes "invar (S,F,T)" and
    ...
    "M = (q_0,f)" and
    "\<not> (apart_machines f (state_run_mealy f q_0 p) f q)" and
    "p \<in> S"
  shows "outs_run_mealy f q inp = outs_run_mealy t p inp"
...
next
  ...
  have "run T p \<noteq> None"
    using assms
    by (smt (verit) Cons.prems(6) case_optionE invars option.discI otree_eq_out run.elims)
  ...
  have pa_not_none: "run T (p @ [a]) \<noteq> None"
    using assms
    by (smt (verit) Cons.prems(6) case_optionE not_rule2 option.discI otree_eq_out run.elims)
  ...
  then have ih_prem: "\<not> apart_machines f (state_run_mealy f q_0 (p @ [a])) f q'"
  ...
  show ?case proof (cases "p @ [a] \<in> S")
    ...
    then have aux: "outs_run_mealy t p (a # inp) = (out # outs_run_mealy t (p @ [a]) inp)"
      using outs_run_mealy_twostep 
      by fast
    then have "outs_run_mealy f q' inp = outs_run_mealy t (p @ [a]) inp"
      using Cons.IH ih_prem True assms(1) assms(2) assms(3) assms(4)
      by blast
    then have "out2 # outs_run_mealy f q' inp = out # outs_run_mealy t (p @ [a]) inp"
      using out2_eq
      by blast
    then have "outs_run_mealy f q (a # inp) = outs_run_mealy t p (a # inp)"
      using aux out2 
  ...
  next
    ...
    have "\<not> apart_machines f (state_run_mealy f q_0 (p')) f q'" proof (rule ccontr)
      assume assapart: "\<not> \<not> apart_machines f (state_run_mealy f q_0 (p')) f q'"
      have "\<exists> inp. drop (length p') (outs_run_mealy f (q_0) (p' @ inp)) \<noteq>
        ...
        then have "\<forall> i. outs_run_mealy f (state_run_mealy f q_0 (p')) i = outs_run_mealy f q' i"
          ...
          by simp
        then have "\<not> apart_machines f (state_run_mealy f q_0 p') f q'"
          by simp
        then show False
          using assapart
          by blast
      ...
      then show False
        ...
        by fastforce
    qed
    then have "outs_run_mealy f q' inp = outs_run_mealy t (p') inp"
      using Cons.IH p' False assms(1) assms(2) assms(3) assms(4) notapart_p'pa
      by blast
    then have "out2 # outs_run_mealy f q' inp = out # outs_run_mealy t (p') inp"
      using out2_eq
      by blast
    then have "outs_run_mealy f q (a # inp) = outs_run_mealy t p (a # inp)" unfolding outs_run_mealy_def 
...
proof (rule ccontr)
  ...
  have "run T s \<noteq> None"
    ...
    by (smt (verit) case_optionE not_None_eq otree.exhaust otree_eq_out)
  then obtain tran op where
  ...
  have "tran i \<noteq> None" proof(rule ccontr)
    ...
    then have "outs_run T (s @ [i]) = None"
      by (smt (verit) Nil_is_append_conv case_optionE not_Cons_self2 option.discI otree_eq_out run_split_none outs_run.elims tranop)
    then show False
      using assms(3) si
      by blast
  qed
...
proof (rule ccontr)
  ...
  have three: "\<not> (apart_machines f (state_run_mealy f q_0 []) f q_0)" unfolding state_run_mealy_def
    by force
